...
async def add_caption(client, message):
    ...
    await AbidAbdullah199.set_prefix(message.from_user.id, prefix)
...
async def delete_prefix(client, message):
    ...
    SnowDev = await message.reply_text("ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ...")
    prefix = await AbidAbdullah199.get_prefix(message.from_user.id)
    if not prefix:
        return await SnowDev.edit("**You Don't Have Any Prefix ❌**")
    await AbidAbdullah199.set_prefix(message.from_user.id, None)
...
async def add_csuffix(client, message):
    ...
    SnowDev = await message.reply_text("ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ...")
    await AbidAbdullah199.set_suffix(message.from_user.id, suffix)
...
async def delete_suffix(client, message):
    ...
    SnowDev = await message.reply_text("ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ...")
    suffix = await AbidAbdullah199.get_suffix(message.from_user.id)
    if not suffix:
        return await SnowDev.edit("**You Don't Have Any Suffix ❌**")
    await AbidAbdullah199.set_suffix(message.from_user.id, None)
